...
function Login() {
  ...
  const [errors, setErrors] = useState([]); // On stocke un tableau d’erreurs
  const navigate = useNavigate();

  const handleLogin = async (e) => {
    e.preventDefault();
    setErrors([]); // On réinitialise les erreurs avant l'appel
    ...
    try {
      ...
      if (err.response) {
        ...
        // Si vous renvoyez un objet { message: "..." }
        if (data.message) {
          setErrors([data.message]); // On place le message dans un tableau
        }
        // Ou si vous renvoyez { errors: [ { msg: "..." }, ... ] }
        else if (data.errors) {
          // On récupère simplement les messages
