...
function New() {
    ...
    const getMeilleurMoment = () => {
        const now = new Date();
        const slots = [new Date(now.setHours(14,30)), new Date(now.setHours(17,0)), new Date(now.setHours(21,45))];
        return slots.find(slot => slot > new Date())?.toISOString().slice(0,16) || new Date(new Date().setDate(new Date().getDate() + 1)).toISOString().slice(0,16);
    };
